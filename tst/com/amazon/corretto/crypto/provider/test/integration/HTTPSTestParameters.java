...
public class HTTPSTestParameters {
  ...
  public static final List<String> SIGNATURE_METHODS_TO_TEST =
      Arrays.asList(
          // SHA1with* is rejected outright by BouncyCastle and/or SunJCE, which is reasonable
          // enough at this point.
          // We'll just not test any of the sha1 cert path cases as they'll all be validated and
          // rejected soon enough.
          // "SHA1withECDSA",
          // "SHA1withRSA",
  ...
  /**
   * Returns {@code true} iff the TLS ciphersuite {@code suite} can be used with certificates signed
   * using {@code signature}.
   *
   * @param suite the TLS ciphersuite
   * @param signatureMethod the algorithm used to sign the TLS certificate
   * @returns true if certificates signed by {@code signature} can be used with {@code suite}}
