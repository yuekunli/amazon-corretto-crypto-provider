...
/**
 * This tool is used to generate the test_CA.jks and test_private_keys.jks files used in the
 * LocalHTTPSIntegrationTest. It simply generates a root CA, plus a series of intermediate-and-leaf
 * certificate pairs, with the leaf cert signatures having a specific size and signing algorithm.
 *
 * <p>Note that we need to use intermediates, as with a single CA all certs directly chained off the
 * root must use a signing algorithm compatible with the root CA type. We interpose the intermediate
 * cert to allow it to use whatever signing algorithm is appropriate for the root, and give the
 * intermediate a key type that is compatible with the final desired signature algorithm.
 */
  ...
  private void generateCertificate(
    ...
    JcaX509v3CertificateBuilder certificateBuilder =
        new JcaX509v3CertificateBuilder(
            issuer, BigInteger.TEN, startDate, expiryDate, subjectName, publicKey);
    ...
    privateKeyStore.setKeyEntry(
        alias, privateKey, SUPER_SECURE_PASSWORD, certChain.toArray(new Certificate[0]));
  }

  private void generateHostCertificate(
      String hostname, String keyType, int keyBits, String signatureAlgo) throws Exception {
    // First generate an intermediate CA with the appropriate key type. We need this because the
    // root's key might
    ...
    String dn =
        String.format(
            "CN=%s,O=TEST CERTIFICATE FOR algo %s bits %d", hostname, signatureAlgo, keyBits);

    generateCertificate(
        hostname, dn, intermediateAlias, keyType, keyBits, signatureAlgo, gn, false);
  }

  private void generateRootCertificate() throws Exception {
    generateCertificate(
        "root", "CN=TEST ROOT CERT", null, ROOT_KEY_TYPE, 521, ROOT_SIGNATURE_ALGO, null, true);
  }
