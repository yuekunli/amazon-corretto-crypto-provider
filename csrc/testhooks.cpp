    ...
    void check_get_result(uint8_t* destbuf, int offset, int off2, int len2)
    {
        for (int i = 0; i < len2; i++) {
            ...
            if (destbuf[i] != (uint8_t)(i + offset + off2)) {
                ...
                int rv = snprintf(message, 120, "Bad value in input array; i=%d val=%02x expect=%02x", i, destbuf[i] & 0xFFU, (unsigned int)expected);
    ...
    JNIEnv* pEnv, jclass, jbyteArray array, jint offset, jint length, jint off2, jint len2)
{
    try {
        ...
        uint8_t* destbuf = reinterpret_cast<uint8_t*>(alloca(std::max((jint)1, len2)));
    ...
    JNIEnv* pEnv, jclass, jbyteArray array, jint offset, jint length, jint off2, jint len2)
{
    try {
        ...
        size_t actual_len = std::max((jint)1, len2);
